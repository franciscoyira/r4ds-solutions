---
title: "Chapter 19 - Excersises - R for Data Science"
...
7. Read the complete lyrics to gLittle Bunny Foo Fooh. Therefs a lot of duplication in this song. Extend the initial piping example to recreate the complete song, and use functions to reduce the duplication.
...
2. Take a function that youfve written recently and spend 5 minutes brainstorming a better name for it and its arguments.
...
1. Whatfs the difference between `if` and `ifelse()`? Carefully read the help and construct three examples that illustrate the key differences
...
2. Write a greeting function that says ggood morningh, ggood afternoonh, or ggood eveningh, depending on the time of day. (Hint: use a time argument that defaults to lubridate::now(). That will make it easier to test your function.)
...
3. Implement a `fizzbuzz` function. It takes a single number as input. If the number is divisible by three, it returns gfizzh. If itfs divisible by five it returns gbuzzh. If itfs divisible by three and five, it returns gfizzbuzzh. Otherwise, it returns the number. Make sure you first write working code before you create the function.
...
```
How would you change the call to `cut()` if Ifd used < instead of <=?
...
6. What does this `switch()` call do? What happens if `x` is geh?
...
2. Itfd be nice if you could supply multiple characters to the `pad` argument, e.g. `rule("Title", pad = "-+")`. Why doesnft this currently work? How could you fix it?
